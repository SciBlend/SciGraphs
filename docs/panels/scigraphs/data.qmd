...
The Data panel imports network data and builds the Blender graph object every
other panel operates on. Which steps appear depends on the selected data source.
...
When a graph object already exists, an **Import Node Attributes** section
attaches a vertex-only attribute table from a separate file.
...
**Setup Visual** adds the Geometry Nodes rendering setup (instanced node spheres
and edge cylinders). It is the same operation documented in the
[Visualization panel](visualization.qmd).
...
layout. This is the global-scale embedding used in the
...
Narrows the import to a slice of a time column. The graph that comes out is a
single graph over the rows that survive, not a sequence.
