...
The Analysis panel computes network measures and stores every result as a named
mesh attribute. It requires a graph to be loaded.
...
Partitions the graph using the SurpriseMe algorithms (`pysurprise`).
...
| **RNSC** (Restricted Neighborhood Search) | King et al., Bioinformatics 20 (2004) |
...
A mode selector at the top switches between analysis modes. The **Surface
Embedding** child subpanel is shown in `Surface` mode.
...
The attributes produced here can drive color and size in the
[Visualization panel](visualization.qmd) or stratify a layout with the
