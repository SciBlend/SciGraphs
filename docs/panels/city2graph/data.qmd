...
The Data Import panel acquires urban feature layers from the shared feature
selector, plus GTFS transit feeds, and materializes them as Blender objects every
other panel can consume.
...
The methods mirror the OSMnx tab so the same workflow works with or without an
imported graph.
...
active OSMnx graph when present.

## Process segments

Prepares a raw segment object for graph construction (splitting by connectors and
extracting barriers). Select the segment object, and optionally also a connectors
object, then run **Process Segments**.
