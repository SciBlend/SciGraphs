...
feature count confirms a valid object). Use **Get as Points** or *Convert to
Centroids* in [Data Import](data.qmd) to prepare point inputs.
:::
...
1. Prepare a **point feature** object in [Data Import](data.qmd).
2. In **Single-Layer**, pick a **Graph Type** and **Metric** (select an OSMnx
   network for **Network** distance), then **Generate Graph**.
3. For cross-type relations use **Multi-Layer**; for containment use
   **Group Nodes**.
4. Tune **Advanced Settings** (deduplication) and **Visualization** (thickness,
   limit, coloring) as needed.
