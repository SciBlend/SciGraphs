# Downloads sparse matrices from https://sparse.tamu.edu and converts them to
# GraphData, either symmetric (A+A^T) or bipartite.
...
def parse_mtx(mtx_text):
    ...
    for i in range(idx, len(lines)):
        ...
        if len(p) < 2:
            continue
        r_idx = int(p[0]) - 1  # the file counts from 1
        c_idx = int(p[1]) - 1
...
def parse_coord_mtx(coord_text):
    ...
    try:
        ...
        # Column-major, so build (D, N) and transpose.
...
def build_symmetric_graph(nrows, ncols, entries, giant_only=True, coords=None):
    ...
    # Row indices index the coordinate array directly.
...
def build_bipartite_graph(nrows, ncols, entries, giant_only=True, coords=None):
    ...
    # Both R and C nodes index the row coordinates: for a symmetric matrix rows
    # and columns are the same entities.
    if coords is not None:
        node_coords = {}
        for n in nodes:
...
def _extract_giant_component(nodes, edges):
    """Return the largest connected component, via union-find."""
    if not edges:
        return nodes, edges
