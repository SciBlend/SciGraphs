# The structure of a declarative pipeline, and validation of user input.
...
SCHEMA = {
    ...
    "visual": {
        ...
        "properties": {
            ...
            # These enums track the operator RNA, not the historical lists.
            # `scripts/repro/spec_doctor.py` re-checks the correspondence.
...
def _apply_defaults(data: Dict[str, Any], schema_section: Dict) -> Dict[str, Any]:
    """Fill in the schema defaults a section did not set."""
    result = dict(data)
...
def validate_pipeline(data: Dict[str, Any]) -> List[str]:
    ...
    if render and not render.get("camera") and not render.get("frame_camera", True):
        warnings.append(
            "Render has neither a named camera nor frame_camera; it will use "
            "whatever camera the scene happens to hold"
        )
...
def parse_spec(data: Dict[str, Any]) -> PipelineSchema:
    """Turn a validated pipeline dictionary into typed dataclasses."""
