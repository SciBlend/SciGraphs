# Loads, parses, canonicalizes and hashes YAML/JSON pipeline files.
...
def load_file(filepath: str) -> str:
    """Read a pipeline file. Raises ParseError if it cannot be read."""
    try:
...
def parse_pipeline(
    ...
    # Return the unresolved dict: the canonical artifact the executor writes has
    # to hash to canonical_hash and stay free of machine-local paths.
    return schema, spec_as_written, canonical_hash
...
def compute_pipeline_hash(data: Dict[str, Any]) -> str:
    """SHA256 hex digest of the canonical pipeline representation."""
...
def save_pipeline(
    ...
    """Write a pipeline schema to disk as 'json' or 'yaml'."""
    data = schema.to_dict()
...
def save_canonical(data: Dict[str, Any], filepath: str) -> str:
    """Write the canonical pipeline JSON and return its hash."""
    canonical = canonicalize_pipeline(data)
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        # Pretty-printed for reading. The hash comes from the canonical form.
