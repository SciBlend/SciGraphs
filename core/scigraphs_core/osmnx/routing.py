...
def _resolve_weight(G, weight, impedance_alpha=5.0):
    """Map the UI weight enum onto a NetworkX edge attribute name."""
...
def calculate_shortest_path(G, orig_node, dest_node, weight="length", impedance_alpha=5.0):
    ...
    try:
        ...
        # ox.shortest_path only warns when an edge lacks travel_time, and then
        # nx.path_weight raises KeyError('travel_time') on it. Check up front.
