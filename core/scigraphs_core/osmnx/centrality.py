...
def _try_rustworkx_digraph(G, weight_attr):
    ...
    try:
        ...
        # Collapse to a simple DiGraph, keeping the lightest parallel edge.
        if G.is_multigraph():
            simple = nx.DiGraph()
...
def edge_betweenness_line(G, weight="length", fast=True):
    ...
    # Edges of the line graph mean incidence, not distance, so they weigh 1.
    if fast:
        try:
            import rustworkx as rx
...
def _fallback_colormap(name, arr_norm):
    """Piecewise-linear colormap over _CMAP_FALLBACK_POINTS. Unknown names give viridis."""
    import numpy as np
