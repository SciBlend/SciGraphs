...
def _fetch_xyz_basemap(
    *,
    ...
    try:
        ...
        interrupted = True
        log("Basemap fetch interrupted by user, canceling pending tiles")
        for f in futures:
            f.cancel()
    finally:
    ...
    if interrupted:
        raise KeyboardInterrupt("Basemap fetch canceled")
...
def _fetch_wms_basemap(
    ...
    metadata = {
        ...
        'bbox_query': bbox,
        # GetMap with CRS=EPSG:4326 returns pixels linear in (lat, lon), so
        # UV mapping is a plain bbox rescale.
